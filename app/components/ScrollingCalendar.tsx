...
export default function ScrollingCalendar({
  indicators,
  monthsPrevious = SCROLLING_CALENDAR_MONTHS_PREVIOUS_DEFAULT,
  monthsNext = SCROLLING_CALENDAR_MONTHS_NEXT_DEFAULT,
  habits,
  ...
  const clampedMonthsPrevious = Math.min(monthsPrevious, 120); //10 years
  const clampedMonthsNext = Math.min(monthsNext, 120); //10 years

  for (let index = clampedMonthsPrevious; index > 0; index--) {
    previousMonths.push(subMonths(firstDayCurrentMonth, index));
  }

  const nextMonths: Date[] = [];
  for (let index = 0; index < clampedMonthsNext; index++) {
        ...
        <div className="md:pr-14 mb-8 md:mb-2">
          <div>
            <MonthScrollingButtons
              monthsPrevious={clampedMonthsPrevious}
